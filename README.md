...
[![License: PMPL-1.0](https://img.shields.io/badge/License-MPL_2.0--1.0-blue.svg)](https://github.com/hyperpolymath/palimpsest-license)

A layered, DANE/TLSA-inspired trust model for CI supply-chain (GitHub
Actions) pinning.

**Status: design / v0** — see
<a href="DESIGN.adoc" class="adoc">DESIGN</a> for the full architecture
and roadmap.
...
(trust-anchor vs end-entity; key vs cert; exact vs hash). Apply the same
layering to actions:

1.  **Anchor** — trust an owner (`actions/*`, `github/*`,
    `hyperpolymath/*`) incl. transitive nested actions.

2.  **Provenance** — admit anything carrying a verified SLSA /
    attestation identity, tolerant of SHA rotation.

3.  **Leaf-exact** — full-SHA pin for the untrusted long tail.
...
`action.yml` + reusable workflows) and emits a layered Anchor /
Provenance / Leaf-exact verdict per node.
...
- <a href="DESIGN.adoc" class="adoc">DESIGN</a> — model, TLSA mapping,
  primitives, scope, roadmap.
